...
### Firewall do Windows: ICMP Time Exceeded (v4 e v6)
Sintoma parecido mas causa totalmente diferente: MTR/Traceroute mostrando `* * *` em **todos** os saltos intermediários, só o salto final (Echo Reply) com dado real. Não é bug de PID/DPI — é o Firewall do Windows descartando a resposta dos roteadores intermediários antes de chegar no raw socket (`tracert`/`ping` nativos não sofrem disso por usarem a API `ICMP.SYS`, fora da mesma avaliação de regras). Diagnosticado em duas sessões reais, uma pra cada família:

- **ICMPv4**: não existe nenhuma regra embutida do Windows liberando entrada de Time Exceeded (tipo 11) — reproduzido numa rede Wi-Fi nova classificada como "Pública".
- **ICMPv6**: existe sim uma regra embutida ("Rede Principal - Tempo Excedido (ICMPv6-In)", tipo 3) e ela aparece **habilitada** — mas com escopo `Program: System` (confirmado com `Get-NetFirewallApplicationFilter` numa sessão real). Ou seja, só libera a mensagem pra tráfego do próprio kernel do Windows, não pra um processo comum como o NOCPing abrindo raw socket. Isso fazia parecer que "IPv6 já funciona por padrão" (era a suposição documentada aqui antes) até alguém de fato rodar MTR contra um destino que resolve IPv6 — mesmo sintoma do IPv4 apareceu de novo. `tracert -6` continuava funcionando normalmente no mesmo instante/rede porque roda como `System` via `ICMP.SYS`, não porque a rede/roteadores estivessem se comportando diferente — o que enganou o diagnóstico inicial até comparar as duas ferramentas lado a lado.

`ensure_icmp_time_exceeded_firewall_rule()` (`core/network.py`) cria as **duas** regras via `netsh` (sem escopo de programa, então cobrem qualquer processo) — chamada por `TracerouteWorker`/`MTRWorker` logo após `is_admin()` passar (mesmo processo já precisa de admin pra rodar essas abas), com guard de módulo (`_firewall_rule_ensured`) pra tentar no máximo uma vez por processo, e falha silenciosamente (nunca deve impedir MTR/Traceroute de rodar — ver README.md, seção "MTR/Traceroute só mostra o salto final", pro fix manual caso a criação automática falhe).
...
- `v2.0.1` — fix de MTR/Traceroute só mostrando o salto final em redes "Públicas" do Windows (falta regra de firewall ICMPv4 Time Exceeded, criada automaticamente agora — ver seção "Firewall do Windows: ICMP Time Exceeded (v4 e v6)"); fix de ícone genérico na barra de tarefas via `WM_SETICON` direto (ver seção "Barra de tarefas: WM_SETICON direto"); `--noupx` no build; documentação do falso-positivo do Windows Defender no README
...
- `v2.0.3` — fix de MTR/Traceroute só mostrando o salto final também em destinos **IPv6** (a regra embutida do Windows pra ICMPv6 Time Exceeded existe mas é restrita a `Program: System`, não cobre a aplicação; `ensure_icmp_time_exceeded_firewall_rule()` agora cria regra própria pra v4 **e** v6 — ver seção "Firewall do Windows: ICMP Time Exceeded (v4 e v6)")
- `v2.0.4` — colunas redimensionáveis (`QHeaderView.ResizeMode.Interactive`) nas `QTableWidget` de MTR/Traceroute/Port Scan/Histórico de RTT, no lugar de `Fixed`; duplo-clique na borda auto-ajusta ao conteúdo (nativo do Qt); largura inicial da coluna IP 150→190px; tooltip com valor completo em toda célula dessas tabelas + Banner/TLS
